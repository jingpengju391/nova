...
type ActionDefinitions = {
    [key: string]: ((payload?: any, options?: { root: boolean }) => any) | any;
}
...
type Commit<M extends MutationRestriction<any, any>> = { <T extends keyof M>(type: T, payload?: Parameters<M[T]>[1], options?: { root: boolean }): void }
export interface RootCommit extends Commit<UnionToIntersection<ModulesMutations>>, Commit<MutationRestriction<BaseMutations>> { }

type DispatchFromDef<A extends ActionDefinitions> = { <T extends keyof A>(type: T, payload?: Parameters<A[T]>[0], options?: { root: boolean }): Promise<ReturnType<A[T]>> }
type Dispatch<A extends ActionRestriction<any, any, any>> = { <T extends keyof A>(type: T, payload?: Parameters<A[T]>[1], options?: { root: boolean }): ReturnType<A[T]> }
