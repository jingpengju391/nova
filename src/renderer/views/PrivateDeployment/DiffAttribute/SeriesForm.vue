<template>
  <el-form ref="form" :model="currentProperty" @submit.prevent>
    ...
    >
      <el-input-number v-model="currentProperty.copySize" :min="1" />
    </el-form-item>
    <el-form-item
      label="允许调整数组大小"
    ...
    <el-form-item
      label="自动求和"
      :class="currentDiffKey === 'isAutoSum' ? 'diff-label' : ''"
    >
      <el-radio-group v-model="currentProperty.isAutoSum" disabled>
        <el-radio :label="true">true</el-radio>
        <el-radio :label="false">false</el-radio>
      </el-radio-group>
    </el-form-item>
    <el-form-item
      v-if="currentProperty.isAutoSum"
      label="自动求和层级"
      :class="currentDiffKey === 'autoSumLevel' ? 'diff-label' : ''"
      disabled
    >
      <el-input v-model="currentProperty.autoSumLevel" />
    </el-form-item>
    <el-form-item
      label="来源"
    ...
    <el-form-item
      label="基准模型"
      :class="
        currentDiffKey === 'rebasePeerModelValueBfRebase' ? 'diff-label' : ''
      "
    >
      <el-checkbox v-model="currentProperty.rebasePeerModelValueBfRebase" />
    </el-form-item>
    <el-form-item
      label="起点重置类型"
    ...
    >
      ...
      >
        <el-option
          v-for="option in rebaseSeriesTypeOptions"
          :key="option.value"
          :value="option.value"
          :label="option.label"
        />
    ...
    >
      <el-checkbox v-model="currentProperty.slidingWindow" />
    </el-form-item>
    <el-form-item
      v-if="currentProperty.slidingWindow"
      label="Sliding Window Chunks"
      :class="currentDiffKey === 'slidingWindowChunks' ? 'diff-label' : ''"
    >
      <el-input-number v-model="currentProperty.slidingWindowChunks" :min="1" />
    </el-form-item>
    <el-form-item
      label="选择分类"
...
<script setup lang="ts" name="SeriesForm">
import {
  rebaseSeriesTypeOptions, isOverrideVisible, sourceOptions, isCalcFormulaVisible,
  currentClassIfy, currentProperty, currentDiffKey
} from '../config'
