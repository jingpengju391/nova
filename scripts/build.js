...
async function generatePackageJson() {
  ...
  const result = {
    ...
    main: './index.js',
    dependencies: Object.entries(original.dependencies).filter(([name, version]) => (original.external.indexOf(name) !== -1 || name === 'socket.io-client')).reduce((object, entry) => ({ ...object, [entry[0]]: entry[1] }), {})
  }
...
async function copyTaskFiles() {
  const taskScriptsSourceDirectory = join(__dirname, '../tasks')
  const taskScriptsDestDirectory = join(__dirname, '../dist/tasks')
  await ensureDir(taskScriptsDestDirectory)
  await ensureDir(join(__dirname, '../tasks/tpl'))
  const fileLists = ['compile.js', 'run.js', 'utils.js', 'tpl']
  for (const file of fileLists) {
    await copy(join(taskScriptsSourceDirectory, file), join(taskScriptsDestDirectory, file))
  }
}
